...
object DefaultNetworkListener {
    ...
    suspend fun get(): Network =
        if (fallback) {
            @TargetApi(23)
            JoyProxyApp.connectivity.activeNetwork ?: error("missing default network")
        } else {
    ...
    private fun register() {
        when (Build.VERSION.SDK_INT) {
            in 31..Int.MAX_VALUE ->
                JoyProxyApp.connectivity.registerBestMatchingNetworkCallback(request, Callback, mainHandler)
            in 28 until 31 ->
                JoyProxyApp.connectivity.requestNetwork(request, Callback, mainHandler)
            in 26 until 28 ->
                JoyProxyApp.connectivity.registerDefaultNetworkCallback(Callback, mainHandler)
            in 24 until 26 ->
                JoyProxyApp.connectivity.registerDefaultNetworkCallback(Callback)
            else ->
                try {
                    fallback = false
                    JoyProxyApp.connectivity.requestNetwork(request, Callback)
    ...
    private fun unregister() {
        runCatching { JoyProxyApp.connectivity.unregisterNetworkCallback(Callback) }
    }
}
